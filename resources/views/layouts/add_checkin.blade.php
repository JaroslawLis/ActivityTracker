...
@section('content')
    <div class="flex-col h-full pt-8 pb-8 pl-8 pr-8 mx-auto bg-white">
        ...
        <x-activitytracker.flash-message />
        ...
        <form action="/add_checkin" method="POST">
            ...
            @foreach ($activities as $activity)
                <div class="p-4">
                    ...
                    <input type="number" name="{{ $activity->id }}" min="0" max="{{ $activity->max_value }}"
                        class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                </div>
            @endforeach
            <input type="data" value="{{ $last_entry_date }}" class="hidden" name="date">
            <button
                class="float-right px-4 py-2 mr-4 font-bold text-white bg-blue-500 rounded hover:bg-blue-700 focus:outline-none focus:shadow-outline">Save</button>
        </form>
    </div>
@endsection
